...
The standalone fork explicitly distinguishes itself from Makepad in [positioning.md](/Users/yuechen/home/octos-one/splash/docs/positioning.md:12). The [upstream Makepad guide](https://github.com/makepad/makepad/blob/dev/splash.md) describes a different UI-host surface; upstream examples should not be treated as the standalone workflow specification.
...
The equality opcode calls recursive `ScriptHeap::deep_eq`, which follows object fields and array elements without a visited-pair set or a work/depth limit. VM call-frame and operand-stack bounds do not measure these Rust calls. Instruction and time sampling happen around VM instructions, so they cannot interrupt work inside this equality operation. See [opcodes_ops.rs](/Users/yuechen/home/octos-one/splash/vendor/makepad/platform/script/src/opcodes_ops.rs:104), [heap.rs](/Users/yuechen/home/octos-one/splash/vendor/makepad/platform/script/src/heap.rs:595), and [vm.rs](/Users/yuechen/home/octos-one/splash/vendor/makepad/platform/script/src/vm.rs:692).
...
In [vm.rs](/Users/yuechen/home/octos-one/splash/vendor/makepad/platform/script/src/vm.rs:652), `handle_errors` unwinds to a `try` when one exists, but otherwise only drains diagnostics. [Evaluation::succeeded](/Users/yuechen/home/octos-one/splash/crates/splash-core/src/lib.rs:780) checks the final value alone. The workflow relies on that predicate before handling output and advancing; see [splash-workflow](/Users/yuechen/home/octos-one/splash/crates/splash-workflow/src/lib.rs:6946).
...
The grammar permits `~`, and [operator_to_unary](/Users/yuechen/home/octos-one/splash/vendor/makepad/platform/script/src/parser.rs:633) maps it to `Opcode::LOG`. The opcode reaches [handle_log](/Users/yuechen/home/octos-one/splash/vendor/makepad/platform/script/src/opcodes_vars.rs:655), bypassing the masked `std.log`, `print`, and `println` members. Its formatter also uses an ordinary growing `String`; see [the logging implementation](/Users/yuechen/home/octos-one/splash/vendor/makepad/platform/script/src/opcodes_vars.rs:737).
...
The canonical parser expresses the expected hierarchy in [profile.rs](/Users/yuechen/home/octos-one/splash/crates/splash-core/src/profile.rs:2600). The inherited parser's short-circuit handling patches an existing `ShortCircuitEnd` without checking its originating operator's precedence; see [parser.rs](/Users/yuechen/home/octos-one/splash/vendor/makepad/platform/script/src/parser.rs:3634).

There is a second precedence mismatch in ordinary comparisons: `true == 2 > 1` yields `false`, while `true == (2 > 1)` yields `true`. The workflow grammar assigns comparisons higher precedence than equality, whereas the VM's [operator table](/Users/yuechen/home/octos-one/splash/vendor/makepad/platform/script/src/parser.rs:518) groups them together.
...
The numeric tokenizer converts through `f64`; see [tokenizer.rs](/Users/yuechen/home/octos-one/splash/vendor/makepad/platform/script/src/tokenizer.rs:369). The VM JSON parser likewise uses floating-point values, and [write_script_json](/Users/yuechen/home/octos-one/splash/crates/splash-core/src/lib.rs:2140) emits the resulting number. Byte limits, depth limits, and JSON syntax validation do not preserve integer identity.
...
- **Assignments as expressions.** `let a = 0; let b = 0; a = b = 3; [a, b]` returns `[nil, 3]`. `set_scope_value` returns `nil`, and assignment forwards that value. The grammar admits nested assignment expressions but does not clearly state the value contract. For this language, either make assignment statement-only and reject chaining, or make assignment expressions consistently return the assigned value. The current behavior silently creates data loss. See [object_heap.rs](/Users/yuechen/home/octos-one/splash/vendor/makepad/platform/script/src/object_heap.rs:645).
...
The companion [evidence JSON](/Users/yuechen/home/octos-one/docs/reviews/splash-language-review-2026-09-04-evidence.json) records sources and observed results. The temporary [probe implementation](/tmp/splash-language-review-20260904/src/main.rs) and Cargo project remain available in this workspace session. Production implementation and tests were not edited.

This review did not run the full all-features workspace gate, cross-platform checks, Miri, sanitizers, or a sustained fuzz campaign. It does not claim complete memory-safety coverage. The existing [syntax fuzz target](/Users/yuechen/home/octos-one/splash/fuzz/fuzz_targets/syntax.rs:67) usefully checks canonical-to-VM acceptance and formatter invariants, but that is not an execution oracle. Replay of the same VM can establish repeatability while repeating the same wrong semantics.
