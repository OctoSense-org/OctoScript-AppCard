...
def pt(s):
    # Sketch writes near-zero coordinates in scientific notation. Splitting
    # the exponent from its mantissa turns {1e-16, 0} into the point (1, -16).
    m = re.findall(r"[+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?", s or "")
...
class Extractor:
    def __init__(self, src):
        ...
        for pid, p in meta["pagesAndArtboards"].items():
            # Page names are presentation, not identity. Light/dark sections
            # can contain identically named pages in the same document.
            self.pages[pid] = json.loads(
    ...
    def text_of(self, layer, overrides, path):
        ...
        return {"string": s, "run": runs[0] if runs else {},
                "vertical_alignment": layer.get('style',{}).get('textStyle',{}).get('verticalAlignment',0)}
    ...
    def node(self, layer, ox, oy, sx, sy, overrides, path):
        ...
        n = {"cls": cls, "name": layer.get("name", ""),
             "object_id": layer.get("do_objectID"), "visible": True,
        ...
        if layer.get('isFlippedHorizontal'): n['flip_x'] = True
        if layer.get('isFlippedVertical'): n['flip_y'] = True
        ...
        for page in self.pages.values():
            ...
            for ab in page.get("layers", []):
                ...
                root = {"cls": "artboard", "name": ab.get("name", ""),
                        "object_id": ab.get("do_objectID"), "visible": True,
