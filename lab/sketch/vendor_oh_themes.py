...
def write_tables(want: dict) -> tuple:
    """Register every vendored mood and axis, generated from what was copied.

    `theme.rs` used to carry these as hand-written macro lists, and they were
    SHORTER than the desktop's: several moods and three whole axis families
    (type, texture, density) had no entry, and `assemble()` skips an entry it
    cannot find without a word. A card naming one of them passed the catalog,
    reached the phone, and rendered as though it had asked for nothing. Deriving
    the tables from the vendored files means the device supports exactly what
    was shipped to it, and a gap becomes a missing FILE rather than a missing
    line someone forgot to add.
    """
    moods, axes = [], []
    for n in sorted(want):
        if n.startswith("_palette_"):
            mood = n[len("_palette_"):-len(".splash")]
            # `dark` is the base itself, so its delta is empty.
            src = '""' if mood == "dark" else f'include_str!("../assets/themes/{n}")'
            moods.append(f'    ("{mood}", {src}),')
        elif n.startswith("_axis_"):
            # `_axis_ground_teal` -> "ground:teal"; `_axis_accent_amber_camo`
            # -> "accent:amber@camo". The mood suffix may itself contain "_".
    ...
    (G.OH / "src" / "theme_tables.rs").write_text(
        "// GENERATED by octos-one/lab/sketch/vendor_oh_themes.py — do not edit.\n"
