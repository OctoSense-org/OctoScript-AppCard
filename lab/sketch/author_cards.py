...
- BOTTOM NAV BAR: almost every app screen has one. Write it as a direct child of view root Surface: TabBar { Tab(icon: .home, label: "Home", active: .on) Tab(icon: .search, label: "Search") Tab(icon: .heart, label: "Saved") Tab(icon: .user, label: "Profile") } — it pins to the page floor as a themed bar with icon-over-label tabs, the active one lit. NEVER draw a nav as a Row of TextCaption("Item"). Give each tab the icon and label the design shows; mark the current screen's tab active.
- COLORED HERO / STAT BLOCK: when the design shows a number or balance on a COLORED panel (a blue spending card, a green total), it is a Card(tint: .blue) { TextEyebrow(text: "October Spending") TextHero(value: total) Row { Chip(text:"Deposit") Chip(text:"Withdraw") } } — never a bare TextHero on the surface. Match the tint to the panel colour (blue green pink amber violet cyan red).
