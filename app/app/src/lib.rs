...
// The standalone entry points (`fn main` on desktop, the JNI `activityOnCreate`
// family on Android) — the same `app_main!` as before, in a module so the
// desktop `main` the macro emits does not warn as dead code in a library. A
// host that mounts the app as a widget builds with `default-features = false`
// and gets no entry point at all.
...
script_mod! {
    ...
    // The app's root body, WITHOUT the `Window{}`: everything the standalone
    // window shows is this one widget, so a host can mount the same body
    // inside its own tree (`host::AppShell`) and the standalone window below
    // mounts it as `body +: { app_body := OctosAppBody{} }`.
    mod.widgets.OctosAppBody = View {
        width: Fill
        height: Fill
        flow: Overlay
        padding: 3
...
/// The standalone root: the app body inside a `Window{}`. Only `app_main!`
/// evaluates this (`AppMain::script_mod`); a host mounting `OctosAppBody` /
/// `AppShell` in its own window never does — `startup()` loads every
/// resource, which a module isolate forbids.
pub mod standalone_root {
    use super::*;
    script_mod! {
        use mod.prelude.widgets.*
        use mod.widgets.*
...
impl App {
    ...
    fn clear_chat(&mut self, cx: &mut Cx) {
        ...
        if let Some(agent) = &mut self.agent {
            ...
            #[cfg(target_os = "android")]
            {
                ...
                // Dev monitor (HTTP on 127.0.0.1:8686, the app's own files
                // dir): the standalone APK only — a host has its own process.
                #[cfg(feature = "standalone")]
                {
    ...
    /// The app's root widget: `Root{Window{..}}` standalone, the bare
    /// `OctosAppBody` when a host mounts it (`host::AppShell`).
    pub fn ui(&self) -> WidgetRef {
        self.ui.clone()
    }

    /// Submit `text` exactly as if it had been typed into the composer and
    /// sent: the same routing (news query on an open L0 card, else the AMA
    /// brain → domain agent → card). The hosting API's `ask`.
    pub fn ask(&mut self, cx: &mut Cx, text: &str) {
        let text = text.trim();
        if text.is_empty() {
            return;
        }
        self.submit_prompt(cx, text.to_string());
    }

    /// Stop the backend: drops the agent, and with it the tokio runtime and
    /// the stdio transport whose kernel child is `kill_on_drop`. The widgets
    /// stay drawable (they just have nothing to talk to); a host calls this
    /// from its instance shutdown before the isolate goes.
    pub fn shutdown_runtime(&mut self) {
        self.approval_handle = None;
        self.task_output_handle = None;
        self.agent = None;
    }
...
impl AppMain for App {
    fn script_mod(vm: &mut ScriptVm) -> ScriptValue {
        ...
        // Every script_mod the app owns (widget prototypes, the kits, the
        // body). A host that already ran the widgets' own `script_mod` calls
        // this one alone.
        crate::host::register_script_mods(vm);
        // The `Window{}` around the body — standalone only.
        crate::standalone_root::script_mod(vm)
    }
